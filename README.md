# 👋 ¡Hola, soy Joselow!

Soy desarrollador **Full Stack** con enfoque en **Vue.js, Node.js y Laravel**.  
Me apasiona construir sistemas escalables y enseñar desarrollo web moderno a través de mi canal [🎥 joselowDev](https://youtube.com/@joselowdev).

---

### 🧠 Stack Principal
- **Frontend:** Vue 3, TailwindCSS, TypeScript  
- **Backend:** Node.js, Laravel, Express  
- **Otros:** Git, Docker, PostgreSQL, Bootstrap  

---

### 🚀 Proyectos Destacados
- [money_move_fronted](https://github.com/Joselow/money_move_fronted) → Frontend Vue + TS para gestión de transacciones.  
- [money_move](https://github.com/Joselow/money_move) → Backend Node + Express + TS.  
- [goals_planner](https://github.com/Joselow/goals_planner) → Planificador de metas con visualización de progreso.

---

### 📫 Conecta conmigo
- 💼 [LinkedIn](https://linkedin.com/in/joshep-manuel-macha-rutti-161127239)  
- 🌐 [Portafolio](https://joselow.github.io/my_portfolio/)  
- 🎥 [YouTube](https://youtube.com/@joselowdev)
---

✨ _“Code. Learn. Share.”_
